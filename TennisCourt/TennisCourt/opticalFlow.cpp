...
//BackgroundSubtractorMOG2 pMOG2(0, 8, false); //MOG2 Background subtractor
...
void processVideo(char* videoFilename) {
  ...
  while( (char)keyboard != 'q' && (char)keyboard != 27 ){
    ...
    //Point start(200, 200);
    //Point end(200, 220);
    //line( frame, start, end, Scalar( 0, 0, 0 ), 2, 8);
    ...
    //goodFeaturesToTrack(frame ,pointsToTrack, 500, 0.01, 10, Mat(), 3, 0, 0.04);
    //for (int i = 0; i < pointsToTrack.size(); ++i) {
    //  cout << "Point: " << pointsToTrack[i] << endl;
    //}
    //cout << pointsToTrack.checkVector(2, CV_32F, true) << endl;
    //cout << "is prev frame same as frame? " << (prevFrame == frame) << endl;
    for (int i = 0; i < pointsToTrack.size(); ++i) {
      //circle(prevFrame, pointsToTrack[i], 4, Scalar(0, 0, 0), -1, 8);
    }
